...
void ClearScreen()
{
	...
	Gdiplus::Graphics    rgraphics(hdc);
	//SolidBrush	GrapBrush(Color(255, 0, 0, 0));
	rgraphics.Clear(Gdiplus::Color(255,255,255,255));
	EndPaint(hMyWnd, &ps);
}
...
int TextBufOut()
{
	...
	double		WHscale=0.5;
#endif	
	Gdiplus::Font        font(&fontFamily, (float)rect.bottom/TextSolutionY*0.882759, Gdiplus::FontStyleBold, Gdiplus::UnitPixel);			//0.882759=font.GetSize()/font.GetHeight(0.0);  这是对应于Courier New字体的值。	
	Gdiplus::SolidBrush	CurBrush(Gdiplus::Color(255, 255, 255, 255));

	Gdiplus::Bitmap		bmpBuf(rect.right,rect.bottom);							//双缓冲
	Gdiplus::Graphics	graphics(&bmpBuf);	
...
int GrapBufOut()
{
	...
	HDC hdc = BeginPaint(hMyWnd, &ps);
	Gdiplus::Graphics    rgraphics(hdc);
	Gdiplus::SolidBrush	GrapBrush(Gdiplus::Color(255, 255, 255, 255));
